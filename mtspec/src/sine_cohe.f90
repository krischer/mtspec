...
!  Freq variables
   ...
   real(4), dimension(:,:), allocatable :: fxin
...
!
   ...
   nptwo = 2*npts
   allocate(fx(nptwo,2))
   allocate(fxin(nptwo,2))

!  ACA

   fxin(1:npts,1) = x1(1:npts)
   fxin(1:npts,2) = x2(1:npts)
   fxin(npts+1:nptwo,1) = 0.0
   fxin(npts+1:nptwo,2) = 0.0

   call fft(fxin(:,1),nptwo,fx(:,1))
   call fft(fxin(:,2),nptwo,fx(:,2))
