...
!	and it will compute the dpss only if necessary. This part 
!	of the code is one of the most time consuming. 
!
!	It looks like Fortran 95 saves in memory all variables in 
!	a subroutine until the main program is closed. I do not 
!	especifically add the save attribute to all my variables, 
!	they are automatycally saved in the subroutine memory. 
!	Not sure if this is the correct way to do it, but seems 
!	to work on the ABSOFT compiler. 
!
...
!	April 2005
!
!  Based on David J. Thomson's Codes, Chave and Thomson's Codes and
...
!  sequences, multiplying each of the kspec taper with the data 
...
!  documentation). By products include the phase information
!  in yk, the eigenspectra sk, the simple weigthed mean spectra sbar, 
!  the jackknife 95% confidence intervals (err), the degrees of freedom 
!  (se) and the weigths wt(nf,kspec) used.
...
subroutine mtspec_d (npts,dt,x,tbp,kspec,nf,freq,spec,verb,qispec, &
		yk, wt, err, se, sk, sbar,                         &
		xi, seavg, rshape, fcrit )
...
!  Simple mean spectra
   ...
   real(8), dimension(nf) 	     :: sbar_o
   real(8), dimension(nf), optional  :: sbar
...
!  Efficiency and Stability of estimate
   ...
   real(8)	     :: xi_o, seavg_o
   real(8), optional :: xi, seavg

!  F test, reshaping

   integer,                   optional  :: rshape
   integer                              :: ier
   real(8),    dimension(nf)            :: F, sline  
   real(4),    dimension(nf)	        :: p
   real(4),                   optional  :: fcrit
   real(4)				:: fcritical
...
!
   ...
   call eigenft(npts, x2, kspec, vn, lambda, nf, yk_o, sk_o, sbar_o)
...
!  double power in positive frequencies
   ...
   if (present(rshape)) then
      if (rshape == 1 .or. rshape == 2) then	! Do not plot or save the lines
...
!
   ...
   seavg_o=sum(se_o)/(2.d0*dble(kspec)*dble(nf))
  
   if (v == 1) then 
      write(6,'(a,g15.7)') ' Average stability of estimate = ', seavg_o
   endif
...
!
   ...
   xi_o=1./(dble(npts)*xi_o)

   if (v == 1) then
      write(6,'(a,g15.7)') ' Variance efficiency = ', xi_o
   endif
!
...
subroutine mtspec_r (npts,dt,x,tbp,kspec,nf,freq,spec,verb,qispec,   &
		yk, wt, err, se, sk, sbar,                           &
		xi, seavg, rshape, fcrit )
...
!  Simple mean spectra
   ...
   real(8), dimension(nf) 	    :: sbar_o
   real(4), dimension(nf), optional :: sbar
...
!
!  Calculate adaptive weigthed spectrum
!
...
!
   ...
   !
      ...
      seavg=sum(se)/(2.d0*dble(kspec)*dble(nf))
...
subroutine mtspec_pad (npts,nfft,dt,x,tbp,kspec,nf,freq,spec,verb,qispec, &
		yk, wt, err, se, sk, sbar,                              &
		xi, seavg, rshape, fcrit )
...
!********************************************************************
   ...
   if (npts >= nfft) then 
      write(6,'(a)') 'For zero padding nfft > npts'
      stop
   endif
...
!
   ...
   call eigenft_pad(npts, nfft, x2, kspec, vn, lambda, nf, yk_o, sk_o, sbar_o)
