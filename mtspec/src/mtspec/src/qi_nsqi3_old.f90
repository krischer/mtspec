...
!  The Quadratic model
   ...
   complex(8), dimension(6)               :: ft_hmodel
...
!  QR Decomposition
   ...
   complex(8), dimension(6)              :: ft_btilde
...
!  QR decomposition
      ...
      call zqrfac(m,6,ft_H,ft_Q,ft_R)
      call zqrcov(6,6,ft_R(1:6,1:6),ft_cov)
...
!     The back transformation (for complex variable)
      ...
      nsspec(j)  = real(ft_hmodel(4))    
      nsdiff(j)  = real(ft_hmodel(5))    
      nsd2(j)    = real(ft_hmodel(6))

      sigma2(j) = sum(abs( C(:,j) - matmul(ft_H,real(ft_hmodel)) )**2)/dble(L-6)
...
!     Don´t allow variance to be smaller than real(4) limit (set to 1e-6)
      ...
      quad_var(j)    = sigma2(j)*ft_cov(3,3)
      nscte_var(j)   = sigma2(j)*ft_cov(4,4)
      nsslope_var(j) = sigma2(j)*ft_cov(5,5)
      nsquad_var(j)  = sigma2(j)*ft_cov(6,6)
