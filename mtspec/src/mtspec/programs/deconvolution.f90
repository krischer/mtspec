...
!
   ...
   integer    ::              nx,nf, nfft
   real(4)    ::              dt 
   real(4), dimension(:), allocatable :: t
   real(4), dimension(:,:), allocatable :: x, spec
   ...
   real(4), dimension(:), allocatable :: cohe, phase
   real(4), dimension(:), allocatable :: spec_ratio, tfun
   
   integer, parameter :: mxx=200000

                             
end module deconv_variables
...
!  Spectral parameters
      ...
      if (allocated(spec_ratio)) then
         deallocate(spec_ratio)
      endif   
      if (allocated(spec)) then
         deallocate(spec)
      endif   
      if (allocated(tfun)) then
         deallocate(tfun)
      endif
      ...
      allocate(spec(nf,2))
      allocate(freq(nf))
      allocate(spec_ratio(nf))
      allocate(tfun(nfft))

      call mt_deconv ( nx,nfft,dt,x(:,1),x(:,2),tbp,kspec,nf,fmax,   &
                     freq,spec_ratio,spec(:,1),spec(:,2),tfun )

      call gplot(freq,spec_ratio,logxy='linlog')
      ...
      allocate(t(nfft))
      t = real( (/(i-1, i=1,nfft)/) )*dt

!      call gplot(t(1:nx),x)

      call gplot(t,tfun,'hold')!,xlimit='6 0.7 1.3')
      call gplot(t,tfun)!,xlimit='6 0.7 1.3',symbol='1')

      print *, 'Energy of STF ','Energy of STF ', sum(tfun**2)

      open(12,file='est_spec.dat')
      do i = 1,nf
         write(12,*) freq(i), spec(i,2), spec(i,2), spec_ratio(i)
      enddo
      close(12)
      open(12,file='est_stf.dat')
      do i = 1,nfft
         write(12,*) t(i), tfun(i)
      enddo
      close(12)
...
!  Saves lines in the input in deconv_variables for later retrieval by
...
!********************************************************************
   ...
   !
     ...
     'tbp NW:      Time-bandwidth product for Thomson method (def=4.)', &
     'fmax fc:    Low-pass filter corner frequency (def=0. no filter)', &
...
!********************************************************************
   ...
   if (0 .eq.  &
      ...
      +index('ntap ntim fmax quit ', com)  &
      +index('kspe tbp meth conf ', com)) then
...
!  Extracts an array of numbers from input in the deconv_variables module.  
!  It is a large array in the deconv_variables module at the end of the 
...
!  a large array in the deconv_variables module at the end of the 
!  file, which has been filled earlier.
...
!  store is a large array in the deconv_variables module at the end
...
!********************************************************************
   ...
   real(4), dimension(40)     :: tab
   real(4), dimension(mxx,2)  :: y
   real(4)                    :: dum
   real(4)                    :: column(2)
