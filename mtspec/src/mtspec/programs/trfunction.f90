...
!  Spectral parameters
      ...
      if (kwit>=0) then
         call gnuplot(freq,cohe,color='1',xlabel='Frequency (Hz)', & 
                    ylabel='Coherence',output='coh_plot.eps')
         call gnuplot(freq,real(trf(1:nf)),'hold',color='2') 
         call gnuplot(freq,imag(trf(1:nf)),color='1',          &
         ...
         call gnuplot(t,real(fx(2:nf)),'hold',color='2')
         call gnuplot(t,imag(fx(2:nf)),color='1')
