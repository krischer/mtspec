...
!
   ...
   real(4), dimension(:,:), allocatable :: dfcohe, dfphase
   complex(4), dimension(:,:), allocatable :: dfspec
   real(4), dimension(:), allocatable :: coher
   
   integer, parameter :: mxx=200000

                             
end module df_variables
...
!  Spectral parameters
      ...
      if (allocated(dfspec)) then
         deallocate(dfspec)
      endif   
      if (allocated(dfcohe)) then
         deallocate(dfcohe)
      endif   
      if (allocated(dfphase)) then
         deallocate(dfphase)
      endif   
      ...
      allocate(freq(nf))
      allocate(dfspec(nf,nf))
      allocate(dfcohe(nf,nf))
      allocate(dfphase(nf,nf))
      allocate(coher(nf))

      call df_spec(nx,dt,x(:,1),x(:,2),tbp,kspec,nf,			&
		   freq,dfspec,dfcohe,dfphase)

      call getchr('save',output,kwit)
      if (kwit>0) then
         output2 = trim(adjustl(output)) // '_cohe.dat'
         open(12,file=output2,form='formatted')
         output2 = trim(adjustl(output)) // '_phase.dat'
         open(13,file=output2,form='formatted')
         write(fmt,'(i12)') nf
         fmt = '(' // trim(adjustl(fmt)) // 'E16.7)'
         do i=1,nf
            write(12,fmt) dfcohe(:,i)
            write(13,fmt) dfphase(:,i)
         enddo 
         close(12)
         close(13)        
      endif
...
!      call gplot(freq,coher)
!
!      call gcolor(dfcohe(1:300,1:300))

      write(6,'(/a)')'Enter further commands or "quit" to terminate run'
      
   enddo

end program dual_freq
...
!  Saves lines in the input in df_variables for later retrieval by
...
!********************************************************************
   ...
   !
     ...
     'tbp nw:                              The time-bandwidth product', &
     'save file:       The file to save the daul-frequaency spectrum ', &
...
!  Extracts an array of numbers from input in the df_variables module.  
!  It is a large array in the df_variables module at the end of the 
...
!  a large array in the df_variables module at the end of the 
!  file, which has been filled earlier.
...
!  store is a large array in the df_variables module at the end
