...
!
   ...
   real(4),    dimension(:), allocatable :: spec_ratio, kopt
   real(4),    dimension(:), allocatable :: speci, specj

   real(4), dimension(:), allocatable :: tfun
...
!  Spectral parameters
      ...
      if (allocated(speci)) then
         deallocate(speci)
      endif
      if (allocated(specj)) then
         deallocate(specj)
      endif
...
!      call mt_deconv ( nx,nfft,dt,x(:,1),x(:,2),tbp,kspec,nf,0.5,     &
!                     freq,tfun,spec_ratio,speci,specj )

      call mt_deconv ( nx,nfft,dt,x(:,1),x(:,2),tbp,kspec,nf,     &
                     freq,tfun,spec_ratio,speci,specj,iadapt,fmax=fmax )

!      call getchr('save',output,kwit)
!      if (kwit>0) then
!         allocate(t(nfft))
!         t = real( (/(i-1-nf, i=1,nfft)/) )*dt
!
!         output2 = trim(adjustl(output)) 
!         open(12,file=output2,form='formatted')
!         do i=1,nfft
!            write(12,'(2E16.7)') t(i), tfun(i)
!         enddo 
!         close(12)
!         deallocate(t)
!      endif
      ...
      if (kwit>=0) then
         ...
         do i = 1,nf
            write(12,*) freq(i), speci(i), specj(i), spec_ratio(i)
         enddo
         close(12)
      ...
      if (kwit>=0) then
         ...
         if (ignor == 2) then
            call gnuplot(t(1:500)-t(500),tfun(nfft-499:nfft), &
               xlimit='6',color='1')
            call gnuplot(tfun(1:500), &
               xlimit='6',color='1')
         else
            call gnuplot(t,tfun,xlimit='6',color='1')
         endif
         deallocate(t)
...
!********************************************************************
   ...
   !
     ...
     'fmax fmax:    Maximum freq. above which the signal is filtered ', &
...
!********************************************************************
   ...
   if (0 .eq.  &
      ...
      +index('ntap ntim fact quit fmax ', com)  &
