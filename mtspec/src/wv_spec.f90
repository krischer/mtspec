subroutine wv_spec ( npts,dt,xi, xj, tbp,kspec,                   &
                     filter, fbox, filename )  
...
!  Filtering matrix
   ...
   complex(4), dimension(:), allocatable :: xout
...
!  Filter vector
   ...
   file1 = trim(filename) // '_wv.dat'
   file2 = trim(filename) // '_df.dat'
   ...
   ! Wigner-Vlle
      ...
      call ifft(x2,npts,xout)

      write(12,fmt) ( real(xout(:)) )
   ...
   deallocate(x2,xout,x_filt,i_filt, df_filt)   
