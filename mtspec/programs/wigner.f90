...
!  Spectral parameters
      ...
      call getint('filter',filter,kwit)
      if (kwit .ne. 1) then
         filter = 0
      endif 
      if (filter>2) then
      ...
      call wv_spec(nx,dt,x,x,tbp,kspec,filter,fbox,output)
...
!  Saves lines in the input in wv_variables for later retrieval by
...
!********************************************************************
   ...
   !
     ...
     'save file:       The file to save the Wigner spectrum (wv_spec)', &
     'filter i:	       The filter to use (0- none 1- Boxcar 2- Gauss)', &
     'fbox width:               The filter width (for filter 1 or 2) ', &
...
!********************************************************************
   ...
   if (0 .eq.  &
      ...
      +index('kspec tbp quit filt fbox ', com)) then
...
!  Extracts an array of numbers from input in the wv_variables module.  
!  It is a large array in the wv_variables module at the end of the 
...
!  a large array in the wv_variables module at the end of the 
!  file, which has been filled earlier.
...
!  store is a large array in the wv_variables module at the end
