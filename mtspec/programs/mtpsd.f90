...
!  Spectral parameters
      ...
      if (kwit>0) then
         ...
         if (0 .eq. index('sine para ',type(1:4))) then
            if (rshape>=0) then 
               call mtspec(nx,dt,x,tbp,kspec,nf,freq,spec,err=err,    &
                           rshape=rshape,fcrit=fcrit,fstat=fstat,     &
                           verb='y',qispec=qi)
            else
               call mtspec(nx,dt,x,tbp,kspec,nf,freq,spec,err=err,qispec=qi)
            endif
         else
...
!  Plot data series
         ...
         if (ignor == 1) then
            call gnuplot(t,x,output='ts.eps')
         endif
...
!  Fill the confidence intervals
         ...
         if (kwit > -90) then
            ...
            call gnuplot(freq2,err2,'hold',fill='fill',logxy='loglog')      
!         else
!            call gnuplot(freq,err,'hold',logxy='loglog')
         endif
         ...
         if (kwit>0) then
            if (0 < index('linlog',pltype(1:6))) then
               print *, 'Doing linlog'
               call gnuplot(freq,spec,logxy='linlog',output=output,   &
   			xlimit=xlimit,ylimit=ylimit)
            elseif (0 < index('linlin',pltype(1:6))) then
               call gnuplot(freq,spec,logxy='linlin',output=output,   &
			xlimit=xlimit,ylimit=ylimit)
            elseif (0 < index('loglin',pltype(1:6))) then
               call gnuplot(freq(2:nf),spec(2:nf),logxy='loglin',     &
			output=output,xlimit=xlimit,ylimit=ylimit)
            else
               call gnuplot(freq(2:nf),spec(2:nf),logxy='loglog',     &
			output=output,xlimit=xlimit,ylimit=ylimit)
            endif
         else 
            call gnuplot(freq,spec,logxy='loglog',output=output,   &
               xlimit=xlimit,ylimit=ylimit)
         endif

         if (rshape>=0) then 
            call gnuplot(freq,fstat,logxy='linlin',output='flinear.eps')
         endif
...
!  Saves lines in the input in main_variables for later retrieval by
...
!********************************************************************
   ...
   !
     ...
     'execute:              With given parameters start mtpsd program', &
     ...
     'skip n:                   Skip over n lines before reading data', &
...
!********************************************************************
   ...
   if (0 .eq.  &
      ...
      +index('kspec tbnw quit fill line plot', com)) then
...
!  Extracts an array of numbers from input in the main_variables module.  
!  It is a large array in the main_variables module at the end of the 
...
!  a large array in the main_variables module at the end of the 
!  file, which has been filled earlier.
...
!  store is a large array in the main_variables module at the end
